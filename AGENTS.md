...
- `static/app.js`: project actions, session tabs and panes, xterm.js integration, authentication UI, reconnect behavior, the mobile key bar, and bell notifications.
...
- **Bell detection uses `term.onBell`, not a scan of the output bytes.** A shell prompt emits a window-title sequence (`OSC … BEL`) on every prompt, so scanning for `\x07` fires constantly; the xterm parser already tells the two apart. Detection stays client-side — the server does not look at terminal content. Notification permission is requested only from the sidebar button: Safari refuses `requestPermission()` outside a user gesture.
