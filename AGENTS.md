...
- `static/app.js`: project actions, session tabs, xterm.js integration, authentication UI, reconnect behavior, the mobile key bar, and bell notifications.
...
- **The `ResizeObserver` watches `#terminal-stack`, not `#terminal-pane`.** The pane is a flex column holding the tab bar, the stack, and the key bar; when the key bar appears or the tab bar grows a horizontal scrollbar, the stack shrinks while the pane's own box does not change — an observer on the pane never fires and the terminals keep a stale size. Observe the box the terminals actually occupy.
